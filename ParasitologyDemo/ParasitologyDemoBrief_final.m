...
babesiosisDir=[pwd,'\BloodSmearImages\babesiosis'];
...
predictedOutcome = trainedClassifier.predictFcn(testInfectionData);
...
%%
index = randi(numel(testSets.Files));
...
thisImageData = array2table(thisImageData,...
	'VariableNames',trainedClassifier.RequiredVariables);
bestGuess = trainedClassifier.predictFcn(thisImageData);
