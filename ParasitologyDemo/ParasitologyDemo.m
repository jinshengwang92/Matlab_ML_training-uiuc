...
%% Explore/Filter Binary Mask in ImageRegionAnalyzer
imageRegionAnalyzer(cellMask) % Explore, export as 'cellMask', generate function
...
cellMask = bwareaopen(cellMask,150);
imshow(cellMask);    
...
showMaskAsOverlay(0.8,morphed1,'r',[],false)
expandAxes(tmpAx);                                                                           
...
% NOTE:
% Cell edges are dilated in refinedMask just for illustrative purposes!
...
%% But not only is this a fairly poor segmentation...it also doesn't generalize well:
togglefig('Babesiosis Images',true)
refreshImages;
for ii = 1:imgSet.Count
	tmpMask = refinedMask2(imgSet,ii);
	showMaskAsOverlay(0.5,tmpMask,'r',true,ax(ii))
	drawnow
end
expandAxes(ax);
...
%%
imtool(rgb2gray(imfuse(targetImage,tmpImg,'montage')))
...
subplot(2,1,2)
h = imshowpair(tempImage,matchedImage,'montage');
title('Image 16;                     Image 16, HistMatched');

%% So now...
imtool(rgb2gray(imfuse(targetImage,matchedImage,'montage')));
...
table({imgSet.Count}','RowNames',{imgSet.Description},...
    'VariableNames',{'Count'})
...
% Consider non-default inputs to bagOfFeatures:

% edit customParasitologyFcn
...
% Re-Train:
classifier = trainImageCategoryClassifier(trainingSets,bag);
% Re-Evaluate the classifier on the test-set images:
[confMat,knownLabelIdx,predictedLabelIdx,predictionScore] = ...
	evaluate(classifier,testSets);
avgAccuracy = mean(diag(confMat));
togglefig('Prediction')
...
togglefig('Prediction')
locations = [testSets.ImageLocation]';
infectionType = repelem({trainingSets.Description}',...
	[testSets.Count], 1);
ii = randi(numel(locations));
img = imread(locations{ii});%read(testSets(ii),randi(testSets(ii).Count));
[labelIdx, predictionScore] = predict(classifier,img);
bestGuess = classifier.Labels(labelIdx);
actual = infectionType{ii};
imshow(img)
...
% infectionType = categorical(repelem({trainingSets.Description}',...
% 	[trainingSets.Count], 1));
% InfectionImageData.infectionType = infectionType;
% 
% save('thisSession','InfectionImageData','imgSet','testSets','trainingSets','bag');
...
%%
% PREDICT the infection type for testSets
index = 0;
goodPrediction = false(sum([testSets.Count]),1);
for ii = 1:size(testSets,2)
    for jj = 1:testSets(ii).Count
        index = index + 1;
        img = read(testSets(ii),jj);
        togglefig('Test Image'); 
        set(gcf,'color','w');
        imshow(img)
        infectionData = double(encode(bag,img));
        if verLessThan('stats','10.1')
            [bestGuess, predictionScore] = predict(trainedClassifier,infectionData);
        else
            infectionData = array2table(infectionData);
            [bestGuess, predictionScore] = trainedClassifier.predictFcn(infectionData);
        end
        % Display the string label for img
        if strcmp(char(bestGuess),testSets(ii).Description)
            titleColor = [0 0.8 0];
            goodPrediction(index) = true;
        else
            titleColor = 'r';
        end
        title(sprintf('Best Guess: %s;\nActual: %s',...
            char(bestGuess),testSets(ii).Description),...
            'color',titleColor)
    end
end
accuracy = nnz(goodPrediction)/numel(goodPrediction);
thisModel = fieldnames(trainedClassifier);
thisModel = thisModel{3};
thisModel = regexprep(thisModel,'Classification','');
sprintf('The accuracy of this %s classifier is %0.2f%% on the TEST set.\n',upper(thisModel),accuracy*100)

%% STILL NOT AS GOOD AS YOU'D LIKE? 
% TRY AGAIN WITH A 2-CLASS MODEL & A CUSTOM EXTRACTOR:
...
table({twoClassSet.Count}','RowNames',{twoClassSet.Description},...
    'VariableNames',{'Count'})
rng(1)
[twoClasstrainingSets, twoClasstestSets] = partition(twoClassSet,0.7,'randomized');
...
% bag = bagOfFeatures(twoClasstrainingSets,...
% 	'CustomExtractor',extractorFcn,...
% 	'StrongestFeatures',1,...
% 	'VocabularySize',1000);
% infectionData = double(encode(bag, twoClasstrainingSets));
% InfectionImageData = array2table(infectionData);
% infectionType = categorical(repelem({twoClasstrainingSets.Description}',...
% 	[twoClasstrainingSets.Count], 1));
% InfectionImageData.infectionType = infectionType;
%
% save('twoClassBag','bag','infectionData','InfectionImageData','infectionType')

load('twoClassBag');
%
classificationLearner % Load InfectionImageData

% EXPORT MODEL -> trainedClassifier %(Linear Discriminant?)
%%
trainedClassifier

%% Again we can use the classifier to PREDICT class membership:
% PREDICT the infection type in a randomly selected test image
index = 0;
goodPrediction = false(sum([twoClasstestSets.Count]),1);
for ii = 1:size(twoClasstestSets,2)
    for jj = 1:twoClasstestSets(ii).Count
        index = index + 1;
        img = read(twoClasstestSets(ii),jj);
        ...
        % Display the string label for img
        if strcmp(char(bestGuess),twoClasstestSets(ii).Description)
            titleColor = [0 0.8 0];
            goodPrediction(index) = true;
        else
            titleColor = 'r';
        ...
        title(sprintf('Best Guess: %s;\nActual: %s',...
            char(bestGuess),twoClasstestSets(ii).Description),...
            'color',titleColor)
    end
end
accuracy = nnz(goodPrediction)/numel(goodPrediction);
